      ...
      <div className="flex pt-16">
              ...
              entries.map((entry) => (
                <div key={entry.userId} onClick={() => setSelected(entry)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-foreground font-medium ${entry.hasProfile ? "cursor-pointer hover:text-lime" : ""}`} onClick={(e) => { e.stopPropagation(); if (entry.hasProfile) navigate(`/creator/${entry.username}`); }}>
                      {entry.username}
                    </span>
                  </div>
            ...
            {loading ? (
              ...
              entries.map((entry) => (
                <div key={entry.userId}
                  onClick={() => entry.hasProfile && navigate(`/creator/${entry.username}`)}
        ...
        {selected && (
          <div className="hidden lg:block w-80 border-l border-border p-6 pt-8 sticky top-16 h-[calc(100vh-4rem)] overflow-y-auto">
            ...
            {selected.hasProfile && (
              <button onClick={() => navigate(`/creator/${selected.username}`)}
